...
internal class Cloud
{
    ...
    public static void Init()
    {
        try
        {
            var content = GetResourcesTxt("TONEX.Resources.Configs.Port.txt");
    ...
    public static bool ShareLobby(bool command = false)
    {
        try
        {
            ...
            Main.NewLobby = false;
            string msg = $"{GameStartManager.Instance.GameRoomNameCode.text}|{Main.PluginShowVersion}|{GameData.Instance.PlayerCount + 1}|{TranslationController.Instance.currentLanguage.languageID}|{ServerManager.Instance.CurrentRegion.Name}|{DataManager.player.customization.name}";
