...
public static class Utils
{
   ...
   public static string GetProgressText(PlayerControl seer, PlayerControl seen = null)
                ...
                switch (role.GetCustomRoleTypes())
                {
                    ...
                    case CustomRoleTypes.Crewmate:
                        ...
                        if (role==CustomRoles.Sheriff)
                            sb1.AppendFormat("\n{0}:{1}", ColorString(GetRoleColor(CustomRoles.Deputy), GetRoleName(CustomRoles.Deputy)), Sheriff.HasDeputy.GetString().RemoveHtmlTags());
    ...
    public static void SendMessageV2(string text, byte sendTo = byte.MaxValue, string title = "<Default>", bool removeTags = false)
    {
        if (AmongUsClient.Instance.AmHost) return;
        if (title == "<Default>") title = "<color=#aaaaff>" + GetString("DefaultSystemMessageTitle") + "</color>";
        Main.MessagesToSend.Add((removeTags ? text.RemoveHtmlTags() : text, sendTo, title + '\0'));
