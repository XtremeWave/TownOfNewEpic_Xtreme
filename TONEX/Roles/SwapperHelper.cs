...
public static class SwapperHelper
{
    ...
    public static bool SwapperMsg(PlayerControl pc, string msg, out bool spam)
    {
        ...
        else if (operate == 2)
        {
            ...
            if (!Swap(pc, target, role, out var reason))
                Utils.SendMessage(reason, pc.PlayerId);
        }
        return true;
    }
    public static bool Swap(PlayerControl swapper, PlayerControl target, PlayerControl target2, CustomRoles role, out string reason, bool isUi = false)
    {
        reason = string.Empty;
        ...
        if (swapper.GetRoleClass() is EvilSwapper esClass || swapper.GetRoleClass() is NiceSwapper nsclass && swapper == target)
        {
            reason = GetString("CantSwapSelf");
            return false;
        }

        Logger.Info($"{swapper.GetNameWithRole()} 交换了 {target.GetNameWithRole()}", "Swapper");

        var dp = guesserSuicide ? swapper : target;
        target = dp;

        Logger.Info($"赌场事件：{target.GetNameWithRole()} 死亡", "Swapper");
        ...
        _ = new LateTask(() =>
        {
            ...
            dp.SetRealKiller(swapper);

            //死者检查
            Utils.NotifyRoles(isForMeeting: true, NoCache: true);

            _ = new LateTask(() => { Utils.SendMessage(string.Format(GetString("SwapKill"), Name), 255, Utils.ColorString(Utils.GetRoleColor(CustomRoles.NiceSwapper), GetString("SwapKillTitle"))); }, 0.6f, "Swap Msg");
    ...
    static void SwapperSelectRole(CustomRoleTypes Role, bool SetPage = true)
    {
    ...
    public static void ShowSwapPanel(byte playerId, MeetingHud __instance)
    {
        ...
        try
        {
            ...
            Teambutton.GetComponent<SpriteRenderer>().sprite = CustomButton.GetSprite("SwapPlateWithKPD");
            ...
            {
                if (!EvilSwapper.OptionCanSwapVanilla.GetBool() && PlayerControl.LocalPlayer.Is(CustomRoles.EvilSwapper) && role.IsVanilla()) continue;
                if (!NiceSwapper.OptionCanSwapVanilla.GetBool() && PlayerControl.LocalPlayer.Is(CustomRoles.NiceSwapper) && role.IsVanilla()) continue;
            ...
            void CreateRole(CustomRoles role)
            {
                ...
                button.GetComponent<SpriteRenderer>().sprite = CustomButton.GetSprite("SwapPlate");
                ...
                if (PlayerControl.LocalPlayer.IsAlive()) button.GetComponent<PassiveButton>().OnClick.AddListener((Action)(() =>
                {
                    ...
                    else
                    {
                        ...
                        Logger.Msg($"Click: {pc.GetNameWithRole()} => {role}", "Swapper UI");

                        if (!PlayerControl.LocalPlayer.IsAlive())
                        {
                            PlayerControl.LocalPlayer.ShowPopUp(GetString("SwapDead"));
                        }
                        else
                        {
                            if (AmongUsClient.Instance.AmHost)
                            {
                                if (!Swap(PlayerControl.LocalPlayer, pc, role, out var reason, true))
            ...
            SwapperSelectRole(CustomRoleTypes.Crewmate);
            ReloadPage();
        }
        catch (Exception ex)
        {
            Logger.Exception(ex, "Swapper UI");
            return;
        }
    }
    ...
    public static void ReceiveRPC(MessageReader reader, PlayerControl pc)
    {
        ...
        if (!Swap(pc, Utils.GetPlayerById(PlayerId), role, out var reason, true))
            pc.ShowPopUp(reason);
    }

    [HarmonyPatch(typeof(MeetingHud), nameof(MeetingHud.OnDestroy))]
    class MeetingHudOnDestroySwapperUIClose
    {
