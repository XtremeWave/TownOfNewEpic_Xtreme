...
public static class NameColorManager
{
    ...
    private static bool KnowTargetRoleColor(PlayerControl seer, PlayerControl target, bool isMeeting, out string color)
    {
        ...
        if (seer.Is(CustomRoles.Jackal) && target.Is(CustomRoles.Attendant)) color = Main.roleColors[CustomRoles.Jackal];
        if (seer.Is(CustomRoles.Attendant) && target.Is(CustomRoles.Attendant)) color = Main.roleColors[CustomRoles.Jackal];
        ...
        if (seer.Is(CustomRoles.Sidekick) && target.Is(CustomRoles.Sidekick)) color = Main.roleColors[CustomRoles.Jackal];

        if (seer.Is(CustomRoles.Whoops) && target.Is(CustomRoles.Jackal)) color = Main.roleColors[CustomRoles.Jackal];
        if (seer.Is(CustomRoles.Jackal) && target.Is(CustomRoles.Whoops)) color = Main.roleColors[CustomRoles.Jackal];
        if (seer.Is(CustomRoles.Whoops) && target.Is(CustomRoles.Whoops)) color = Main.roleColors[CustomRoles.Jackal];

        if (seer.Is(CustomRoles.Whoops) && target.Is(CustomRoles.Sidekick)) color = Main.roleColors[CustomRoles.Jackal];
        if (seer.Is(CustomRoles.Sidekick) && target.Is(CustomRoles.Whoops)) color = Main.roleColors[CustomRoles.Jackal];
        if (seer.Is(CustomRoles.Whoops) && target.Is(CustomRoles.Whoops)) color = Main.roleColors[CustomRoles.Jackal];
